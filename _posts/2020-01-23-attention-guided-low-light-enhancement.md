...
- **Noise-Net loss**  
  ...
  $${A}'=\mathcal{F}_{a}(I)$$이고, $$\mathcal{F}_{n}(I,{A}')$$는 Noise-Net이 추론한 Noise map, $N$은 예상되는 Noise map입니다. Noise map의 ground truth 영상을 구하는 방법은 논문에 나와 있지 않지만 dataset을 구성하는 단계에서 노이즈를 부여하기 전/후 상황을 이용하여 구할 수 있습니다.
